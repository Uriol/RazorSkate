...
//    Serial.print(rz); Serial.println();
  ...
  // change gravity value *-1 so I can substract
//  gx = gx*-1;
//  gy = gy*-1;
...
//  Serial.print(finalSpeedX); 
  ...
  if (_roll >= -10 && _roll <= 10) {
      gravity_y_Off = true;
      Serial.println(_roll);
     // Serial.println("roll with no gravity");
  } else { gravity_y_Off = false; 
  // Serial.println("roll with gravity"); 
  }
  
  // Calculate when y started moving
  if ( YstartedMoving == true ) {
    //Serial.println("Y started moving");
  } else if ( initialSpeedY >= 0.05 || initialSpeedY <= -0.05 ) {
    YstartedMoving = true;
    //Serial.println("Y started moving first time");
  } else { //Serial.println("Y not moving yet");
 }
  
  
 // Check when both conditions are happenning
 if ( YstartedMoving == true &&  gravity_y_Off == true ) {
  // Serial.println("gravity not counting");
   cy = ry;
 } else { cy = gy + ry; 
// Serial.println("gravity substracted");
